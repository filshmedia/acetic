...
/**
 * Copies the source file to the destination path. If
 * createDirectories is true, it will create the directory
 * structure before copying
 * @param  {String} source
 * @param  {String} destination
 * @param  {Boolean} createDirectories (default: false)
 * @param  {Function} callback
 */
function copyFile(source, destination, createDirectories, callback) {
  if (typeof createDirectories === 'function') {
    callback = createDirectories;
    createDirectories = false;
  }

  if (createDirectories) {
    var destinationDir = path.dirname(destination);
    ensurePathExists(destinationDir);
  }
  ...
  var rs = fs.createReadStream(source).pipe(ws);
}
