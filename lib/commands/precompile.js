...
/**
 * Finds the files it should browserify and browserifies them
 * @param  {String}   assetType
 * @param  {Object}   files
 * @param  {Function} callback
 */
Precompile.prototype.browserifyFiles = function(assetType, files, callback) {
  if (assetType !== 'javascripts' || !this.options[assetType].browserify) return callback(null);

  var affectedFiles = Object.keys(files)
    , self = this;

  async.eachSeries(affectedFiles, function (file, callback) {
    self.browserify(file, callback);
  }, function (err) {
    callback(err)
  });
};
...
/**
 * Browserifies the given asset
 * @param  {String}   destinationFile
 * @param  {Array}    sourceFiles
 * @param  {Function} callback
 */
Precompile.prototype.browserify = function(file, callback) {
  var assetOptions = this.options.javascripts
    , filePath
    , self = this;
