...
Middleware.prototype.init = function (req, res, next) {
  ...
  for (var i = 0; i < assetTypes.length; i++) {
    ...
    for (var j = 0; j < assetCompiler.sourceExtensions.length; j++) {
      ...
      var assetSourcePath = sourcePath.replace(
        new RegExp("\." + assetCompiler.destinationExtension + "$"), // e.g. \.js$
        "." + extension
      );
    ...
    if (!sourceExists) {
      ...
      if (fs.existsSync(sourcePath)) {
        ...
        utils.copyFileSync(sourcePath, requestedPath, true, function () {
      ...
      return this.serveOrCompileAsset(requestedPath, assetSourcePath, assetCompiler, next);
    }
  }
